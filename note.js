...
03.轮播图
    ...
    (问题)
      ...
      3.2 (未解决)如果刷新了页面，鼠标正好在轮播图里面，它就没有停止自动播放
          ...
          (判断鼠标是否在dom元素里面)还有一个是，或者left 和 left + offsetWidth 以及top和offsetHeight，但是发现父元素都有定位，导致获取的offsetWidth都是0，尝试获取offsetParent，发现还是定位，那么就不太可取了
...
10. (问题)最新模块
      ...
      (贴合) 注意flex的使用
11. 图片懒加载的使用
      1. 核心是: new IntersectionObserve(([{isIntersecting}]) => {})
      2. isIntersecting是表示 是否交叉 如果交叉
         停止监视dom
         error事件赋值默认的图片
         其他都是正常的图片 用 binding.value
